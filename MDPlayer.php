...
use pocketmine\{
    ...
    math\Vector3,
    math\Vector2,
    network\mcpe\NetworkSession,
    network\mcpe\protocol\ChangeDimensionPacket,
    network\mcpe\protocol\UpdateBlockPacket,
    network\mcpe\RakLibInterface,
    Player,
...
class MDPlayer extends Player{
    ...
    public function __construct(RakLibInterface $interface, string $ip, int $port){
        ...
        if (($plugin = $this->getServer()->getPluginManager()->getPlugin("LegacyHCF")) instanceof Myriad && $plugin->isEnabled()) {
    ...
    public function checkSets()
    {
        if ($this->isBard()) {
            ...
            //Speed
            $speed = new EffectInstance(Effect::getEffect(Effect::SPEED), 20 * 9999999, 0);
            $this->addEffect($speed);
            //Regeneration
            $reg = new EffectInstance(Effect::getEffect(Effect::REGENERATION), 20 * 9999999, 0);
            $this->addEffect($reg);
            //Resistance
            $res = new EffectInstance(Effect::getEffect(Effect::RESISTANCE), 20 * 9999999, 0);
            $this->addEffect($res);
            //Fire res
            $fres = new EffectInstance(Effect::getEffect(Effect::FIRE_RESISTANCE), 20 * 9999999, 1);
            $this->addEffect($fres);
        } else
        if ($this->isMiner()) {
            $this->setClass("miner");
            $this->removeAllEffects();
            //NightVision
            $nv = new EffectInstance(Effect::getEffect(Effect::NIGHT_VISION), 20 * 9999999, 3);
            $this->addEffect($nv);
            //Haste
            $haste = new EffectInstance(Effect::getEffect(Effect::HASTE), 20 * 9999999, 1);
            ...
            $this->addEffect($fres);
             //Invisibilty
            if($this->getY() < 30) {
                $inv = new EffectInstance(Effect::getEffect(Effect::INVISIBILITY), 20 * 5, 1);
                $this->addEffect($inv);
            }
        ...
        if ($this->isArcher()) {
            ...
            $this->removeAllEffects();
            //Speed
            $speed = new EffectInstance(Effect::getEffect(Effect::SPEED), 20 * 9999999, 2);
            $this->addEffect($speed);
            //Regeneration
            $reg = new EffectInstance(Effect::getEffect(Effect::REGENERATION), 20 * 99999, 0);
            $this->addEffect($reg);
            //Resistance
            $res = new EffectInstance(Effect::getEffect(Effect::RESISTANCE), 20 * 99999, 1);
            $this->addEffect($res);
        } else {
    ...
    public function isArcher(): bool
    {
        ...
        return ($helmet == Item::LEATHER_HELMET && $chestplate == Item::LEATHER_CHESTPLATE && $leggigns == Item::LEATHER_LEGGINGS && $boots == Item::LEATHER_BOOTS);
    }
    ...
    {
        if($value){
            $this->getPlugin()->getServer()->getAsyncPool()->submitTask(new SaveDataTask($this->getName(), "DeathBanned", 1, SaveDataTask::SET));
        } else {
            $this->getPlugin()->getServer()->getAsyncPool()->submitTask(new SaveDataTask($this->getName(), "DeathBanned", 0, SaveDataTask::SET));
        }
    }
    ...
    */
    ...
    public function checkClaim()
    {
        ...
        $this->getPlugin()->getServer()->getAsyncPool()->submitTask(new CheckClaimTask($x1, $z1, $x2, $z2, $dir, $this->getName()));
    }
    ...
    {
        $this->getPlugin()->getServer()->getAsyncPool()->submitTask(new CheckProxyTask($this->getName(), $this->getAddress()));
    }
