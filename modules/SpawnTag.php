...
class SpawnTag implements Listener
{
     ...
     * @param Player $player
     */
    public function buildSpawnBorder(Player $player)
    {
        $y = 66;
        ...
        for($x = $this->minX; $x <= $this->maxX; ++$x){
            for($z = $this->minZ; $z <= $this->maxZ; ++$z) {
                $blocks = [
                    (Block::get(Block::STAINED_GLASS, 14)->setComponents($x, $y, $z)->setLevel($player->getLevel())),
                    (Block::get(Block::STAINED_GLASS, 14)->setComponents($x, $y + 1, $z)->setLevel($player->getLevel())),
                    (Block::get(Block::STAINED_GLASS, 14)->setComponents($x, $y + 2, $z)->setLevel($player->getLevel()))
                ];
                $player->getLevel()->sendBlocks([$player], $blocks, UpdateBlockPacket::FLAG_ALL);
            }
        }
    }
